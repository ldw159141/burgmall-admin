<template>
    <div>
      ...
      <div style="margin-top: 20px; border: 0px solid red;margin-bottom: 20px;">
        类别名：<el-input type="text" v-model="searchForm.name" @input="handleSearch" class="input" clearable placeholder="请输入类别名" size="small" style="width: 200px;margin-right: 20px;"></el-input>
  ...
  export default {
    ...
    methods: {
      //分页
      list() {
        let data=Object.assign(this.searchForm,this.typeQuery)
        this.axios.post("type/page",data).then((res) => {
            console.log(res);
          this.tableData = res.data.list;
          this.typeQuery.total=res.data.total
        });
      },
      //点击跳转页面，更改pageNum
      changePage(e) {
        this.typeQuery.pageNum = e
        this.list()
      },
  ...
  this.ids=[]
      ...
      bathDelete() {
        ...
        //批量删除数据
            this.axios.post('type/delete', this.ids).then(res => {
              if (res.code === 200) {
                this.$message({
              type: 'success',
              message: '删除成功!'
      ...
      submit() {
        this.$refs.submitForm.validate((valid) => {
          if (valid) {
            this.axios.post("type/save", this.submitForm).then((res) => {
              console.log(res);
              if (res.code === 200) {
                this.submitForm = {};
                this.$message({
